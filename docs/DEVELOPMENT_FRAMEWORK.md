...
对话输入先经过本地任务路由。路由结果至少包含意图、范围、操作、副作用、分析模式、覆盖范围、来源策略和是否需要模型。分析模式为 Runtime 内部的“概览分析（Overview Analysis）”与“深度分析（Deep Analysis）”，不作为用户主动选择的输入选项；路由按问题是否需要正文语义自动判断。覆盖范围独立为 `index-only`、`targeted`、`exhaustive`。`StructureSegmentation` 直接在源文档同级的 `<源文件名>-章节拆分/` 输出编号文件，不要求配置模型；普通聊天、摘要和语义归纳只有在路由确认需要时才组装上下文并访问模型。首轮拆分完成后，用户可主动确认进入 `structure-enhancement` 进行 AI 重梳理；增强结果默认不覆盖首轮输出。
...
自动分类以可回答性为准：目录、文件名、数量、类型和索引状态属于概览；作品内容、项目用途、主题、人物、关系、情节、主线、设定、风格、摘要和综合分析属于深度。概览上下文必须要求模型先回答可确认的结构事实，不能因语义摘要不可用而整体拒答；真正需要语义摘要的问题在首次路由时直接进入深度，不通过模型拒答后的自动重试来升级权限。
