...
纯元数据概览由 `WorkspaceIntelligenceService` 直接格式化确定性回答，不依赖模型；因此即使不存在缓存摘要，也能回答目录、清单、数量和类型问题。摘要不可用不能导致整个概览请求被拒答。
...
| `ResearchAgent` | 外部资料检索、事实核查和来源整理；默认关闭，单独显示联网范围；实现边界见[轻量级联网搜索设计](../../research/LIGHTWEIGHT_WEB_RESEARCH.md) |
