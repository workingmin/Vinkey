...
| 4. W0 脚本 | 壳层 P、响应式 P、导航 F、编辑器 F | `.sh`、`.ps1`、npm 入口、fixture 生成器 | 从仓库外目录可重复执行；不写用户目录 | 开发 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `W0-SHELL-P` | `D-SHELL` 应用壳层 | 无 | `UI-ACC-SHELL-001` | store、原生窗口宽度同步已有；App/TitleBar 直接渲染覆盖仍缺 | 编写跨平台窗口/菜单诊断脚本；浏览器不可作为桌面结论 | 标题栏、菜单、窗口控制、侧栏容器、设置承载 | 待编写，可先行 |
| `W0-RESP-P` | `Q-RESP` 响应式/主题 | 壳层 Token | `UI-ACC-RESPONSIVE-001` | token、部分渲染和可访问性测试 | 提供窗口尺寸采集、焦点检查和截图命名辅助，不调用 Ollama | 三种桌面尺寸、窄窗口、主题、焦点和无重叠 | 待编写，可先行 |
