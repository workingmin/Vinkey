...
- 验收报告：[UI_ACCEPTANCE_SETTINGS.md](./acceptance/UI_ACCEPTANCE_SETTINGS.md)
...
本功能域按统一生命周期执行，具体证据链、执行命令和遗留风险见[模型设置专项验收报告](./acceptance/UI_ACCEPTANCE_SETTINGS.md)。本页的组件 ID、入口 ID 和实现路径是报告反向追踪的设计基线。
