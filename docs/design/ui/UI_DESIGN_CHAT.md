...
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr><td bgcolor="#F1F3F5"><strong>会话摘要</strong>　会话标题　工作区 · 模型　　　　　　　<strong>对话</strong>　|　文件　|　日志</td></tr>
  <tr>
...
- 当前实现用首次有效用户输入合并空白后截取 28 个字符作为确定性临时标题；它不是持续摘要，也不应被描述为“最后一轮问题标题”。语义标题升级、手动重命名和失败回退由[项目与会话导航设计](./UI_DESIGN_NAVIGATION.md#43-会话标题生命周期)定义，当前仍待实现和验收。

## 组件与业务功能映射

本表是对话页面验收清单；跨到日志中心或编辑器的组件仍登记在用户当前看到并操作它的页面。
