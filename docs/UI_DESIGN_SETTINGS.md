...
设置页用于选择默认模型、管理模型服务和查看本机配置。底层仍会检查模型能否支持应用所需的响应格式，但页面统一使用“检查”“可用”等用户语言，不呈现 Skill、Schema、格式准入等实现术语。
...
<table width="100%">
  <tr>
    <td colspan="2">
      ...
      <small>选择默认模型并管理模型服务　<kbd>返回工作区</kbd></small>
    </td>
  </tr>
  <tr>
    <td width="72%" valign="top">
      <h3>默认模型</h3>
      <p>对话、续写、改稿等功能将使用这个模型。</p>
      <p>
...
- 已保存但没有有效检查结果的默认模型仍显示，并标记“需要检查”，避免重开应用后静默替换用户选择。
