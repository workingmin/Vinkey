...
| `CF-024` | 系统标题栏与功能菜单语义 | CloudCLI、Cherry Studio、NoteGen、Obsidian、Typora、Cursor；逐项证据卡见 `TITLE_BAR_DESIGN.md` | 项目/工作区、会话、文档和编辑器是不同层级；成熟 AI/本地工作台把项目与会话作为稳定控制面，单文档编辑器的文件菜单不能直接覆盖多对象工作台 | 一级菜单采用“项目/会话/编辑/查看/窗口/帮助”；文件新建/保存/关闭回到项目文件页局部入口；编辑命令按当前焦点分派；设置移出“查看”；Windows 自绘、macOS 原生菜单保持共同语义 | `设计已决策 / 实现待对齐` | `UI_DESIGN_SHELL.md`、`TITLE_BAR_DESIGN.md`、`UI_ENTRY_POINTS.md`、`UI_ACCEPTANCE_SHELL.md`；当前 `src/App.tsx` 仍为旧“文件”菜单，未完成前不得按目标菜单验收通过 |
