...
复制本文件后，将文件名改为稳定的英文产品名；详细报告放在 `docs/competitors/`，不要把一次体验直接写入架构结论。新报告必须先通过[竞品筛选方法论](SELECTION_METHODOLOGY.md)，并使用[正式竞品名录](COMPETITOR_CATALOG.md)和[竞品术语规范](TERMINOLOGY.md)中的规范名称。
...
| 条件 | 证据 URL / 日期 | 是否满足 |
| --- | --- | --- |
| 公开可用或正式上线至少 90 天 |  | 是 / 否 |
| 最近 180 天至少 2 次实质性维护 |  | 是 / 否 |
| 产品内用户反馈 |  | 是 / 否 |
| 产品外独立反馈 |  | 是 / 否 |
| 市场认可信号 |  | 是 / 否 |
| 许可证/商业条款可复查 |  | 是 / 否 |
