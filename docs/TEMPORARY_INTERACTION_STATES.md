...
- 会话栏、当前对话输入区和助手消息使用同一份状态，不各自维护副本。
...
- 会话标题下显示状态短标签、呼吸点和可选进度；状态存在时会话图标使用强调色。
- 状态行保留消息数和更新时间，长文本或窄侧栏下使用省略号，不改变会话项高度。
- 后台会话也显示状态；用户切换回来时恢复同一运行记录。
- `stopping` 使用警示色但不显示为错误；`awaiting_approval` 使用可操作的确认样式。

### 对话区和输入区

- 当前助手气泡保留打字动画；有内容后显示流式光标，不重复堆叠多个加载动画。
- 输入区提示与会话栏使用同一状态源；运行期间发送按钮变为停止，`stopping` 时禁用并显示等待。
...
- 发送、长文本分析、流式输出和停止操作均能在会话栏看到对应阶段，且状态不会残留。
...
- 未来接入 Agent Runtime 时，只需消费 `TaskEvent`，不改造会话栏布局和视觉 token。
