...
import type { TaskIntent, TaskScope, TaskSideEffect } from './intent'
...
export interface SkillDefinition {
  ...
  description: string
  allowedTools: string[]
  contextScopes: Array<'none' | TaskScope>
  sideEffects: TaskSideEffect[]
...
const skillDefinitions: SkillDefinition[] = [
  {
    ...
    allowedTools: ['list_document_profiles', 'get_document_digest', 'stream_chat'], contextScopes: ['selected-documents'], sideEffects: ['draft'],
  ...
  },
  {
    ...
    allowedTools: ['get_workspace_profile', 'list_document_profiles', 'get_project_digest'], contextScopes: ['workspace'], sideEffects: ['draft'],
  ...
  },
  {
    ...
    contextScopes: ['workspace'], sideEffects: ['draft'], approvalPolicy: 'auto', modelRequirements: 'configured',
    failureAndRetry: '证据不足时明确覆盖范围并建议深度分析，不自动扩大读取目标。',
  },
  {
    name: 'general-conversation', version: '1.0.0', description: '普通对话和创作建议，不写入工作区',
    allowedTools: ['stream_chat', 'search_project_memory'], contextScopes: ['conversation'], sideEffects: ['draft'],
  ...
  },
  {
    ...
    allowedTools: ['read_document', 'create_directory', 'create_document'], contextScopes: ['selected-documents'], sideEffects: ['proposal'],
  ...
  },
  {
    ...
    allowedTools: ['read_document', 'stream_chat'], contextScopes: ['selected-documents'], sideEffects: ['draft'],
  ...
  },
  {
    ...
    allowedTools: ['read_document', 'chunk_document', 'stream_chat', 'write_analysis_artifact', 'read_analysis_artifact', 'list_analysis_jobs', 'search_project_memory'], contextScopes: ['selected-documents'], sideEffects: ['draft'],
  ...
  {
    name: 'continuity-review', version: '1.0.0', description: '检查人物状态、时间线、设定和伏笔的一致性并输出证据报告',
    allowedTools: ['read_document', 'chunk_document', 'stream_chat', 'write_analysis_artifact', 'read_analysis_artifact', 'list_analysis_jobs', 'search_project_memory'],
    contextScopes: ['selected-documents', 'workspace'], sideEffects: ['draft'], approvalPolicy: 'auto', modelRequirements: 'configured',
    failureAndRetry: '证据不足时标记未知，不把缺失信息推断为冲突；保留已完成阶段并允许恢复。',
  },
  {
...
const taskCapabilities: Record<TaskIntent, TaskCapabilities> = {
  ...
  'document-revision': { agent: 'RevisionEditor', skill: 'document-revision', allowedTools: ['read_document', 'chunk_document', 'stream_chat', 'write_analysis_artifact', 'read_analysis_artifact', 'list_analysis_jobs', 'search_project_memory'] },
  ...
  'continuity-review': { agent: 'ContinuityReviewer', skill: 'continuity-review', allowedTools: ['read_document', 'chunk_document', 'stream_chat', 'write_analysis_artifact', 'read_analysis_artifact', 'list_analysis_jobs', 'search_project_memory'] },
...
export function listSkillDefinitions(): SkillDefinition[] {
  return skillDefinitions.map((skill) => ({ ...skill, allowedTools: [...skill.allowedTools], contextScopes: [...skill.contextScopes], sideEffects: [...skill.sideEffects] }))
}
